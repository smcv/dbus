...
{
  DBusOpenStandardFdsFlags flags;
  int ret;
  const char *s;
  ...
  flags = DBUS_OPEN_STDOUT_NULL;

  if (!exit_with_session)
    flags |= DBUS_OPEN_STDIN_NULL;

  s = getenv ("DBUS_DEBUG_OUTPUT");

  if (s == NULL || *s == '\0')
    flags |= DBUS_OPEN_STDERR_NULL;
   ...
   */
  if (!_dbus_open_standard_fds (flags, &s))
    {
      fprintf (stderr, "%s: %s\n", s, strerror (errno));
      exit (1);
    }
...
do_close_stderr (void)
{
  ...
  if (!_dbus_open_standard_fds (DBUS_OPEN_STDERR_NULL, &err))
    {
      fprintf (stderr, "%s: %s\n", err, strerror (errno));
      exit (1);
    }
...
main (int argc, char **argv)
{
  ...
  if (!_dbus_open_standard_fds (0, &error_str))
    {
      fprintf (stderr,
